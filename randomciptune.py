...
def draw_piano(screen):
    ...
    for i, (note, frequency) in enumerate(notes.items()):
        ...
        text_rect = text.get_rect(center=(i * key_width + key_width // 2, SCREEN_HEIGHT // 2))
...
def generate_and_play_sound(note, duration, start_time, tempo=120):
    frequency = notes[note]
    square_wave = generate_square_wave(frequency, duration * 60.0 / tempo)
    sound = pygame.sndarray.make_sound(square_wave)
    sound.play()
...
# List to store currently playing sounds
playing_sounds = []
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                # Left click
                scroll_x = add_note_on_click(pygame.mouse.get_pos()[0], melody, scroll_x)
